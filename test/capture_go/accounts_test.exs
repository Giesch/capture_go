defmodule CaptureGo.AccountsTest do
  ...
  describe "authenticate_by_username_and_password/2" do
    ...
    setup do
      user = user_fixture(username: @username, password: @pass)
      {:ok, user: user}
    end

    test "returns user with correct password", %{user: %User{id: id}} do
      result = Accounts.authenticate_by_username_and_password(@username, @pass)
      assert {:ok, %User{id: ^id}} = result
    end

    test "returns unauthorized error with invalid password" do
      result = Accounts.authenticate_by_username_and_password(@username, "badpass")
      assert {:error, :unauthorized} == result
    end

    test "returns not found error with no matching user for username" do
      result = Accounts.authenticate_by_username_and_password("badmail@localhost", @pass)
